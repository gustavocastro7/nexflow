...
import type { RawInvoice, CostCenter, ImportPreview } from '../types';
...
export default function InvoicesPage() {
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      ...
      fetchRawInvoices();
    } catch { showError('Erro ao remover fatura'); }
    ...
    <Container maxWidth={false} sx={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column', py: 2 }}>
                ...
                <List disablePadding>
                  {rawInvoices.map((raw) => (
                    <ListItemButton key={raw.id} selected={selectedRaw?.id === raw.id}
                      ...
                      <ListItemText primary={`Fatura ${raw.operator.toUpperCase()}`}
      ...
      <Dialog open={importDialogOpen} onClose={handleCloseImportDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {importStep === 'select' && (
            <Stack spacing={2} sx={{ mt: 1 }}>
              <Box>
                <Typography variant="body2" sx={{ fontWeight: 700, mb: 0.5 }}>Centro de custo:</Typography>
              ...
              <Box>
                <Typography variant="body2" sx={{ fontWeight: 700, mb: 0.5 }}>Arquivo da fatura:</Typography>
          ...
          {importStep === 'importing' && importResult && (
            <Box sx={{ textAlign: 'center', py: 3 }}>
              <Typography variant="h6" sx={{ color: '#10B981', fontWeight: 700, mb: 1 }}>Importação concluída!</Typography>
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          {importStep === 'importing' && importResult && (
            <Button onClick={handleCloseImportDialog} variant="contained" sx={{ bgcolor: '#10B981' }}>Concluído</Button>
          )}
        </DialogActions>
      </Dialog>
