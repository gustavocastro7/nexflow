...
import {
  ...
  TableHead, TableRow, Chip, Stack, alpha, useTheme, Skeleton, Box,
} from '@mui/material';
...
const InvoiceList = memo<InvoiceListProps>(({
  ...
  useEffect(() => {
    if (containerRef.current) setRootEl(containerRef.current);
  }, []);
    ...
    <Paper sx={{ overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {loading && !isInitialLoading && !hasMore && (
        <Box sx={{
          ...
          bgcolor: alpha(theme.palette.primary.main, 0.2), zIndex: 10,
          '&::after': {
            content: '""', position: 'absolute', top: 0, left: 0, bottom: 0,
            width: '30%', bgcolor: theme.palette.primary.main,
      ...
      <TableContainer ref={containerRef} sx={{ flex: 1, overflow: 'auto', contain: 'content' }}>
              ...
              <>
                {invoices.map((inv) => {
                  ...
                  return (
                    <TableRow key={inv.id} hover selected={isSelected}
                      ...
                      sx={{ cursor: 'pointer', '&.Mui-selected': { bgcolor: alpha(theme.palette.primary.main, 0.1) } }}
                    >
                      <TableCell>
                        <Chip label={carrier.label} size="small"
                          sx={{ bgcolor: alpha(carrier.color, 0.1), color: carrier.color, fontWeight: 700, borderRadius: 1 }} />
                      </TableCell>
                      <TableCell sx={{ fontWeight: 700, whiteSpace: 'nowrap' }}>{inv.source_phone}</TableCell>
                      <TableCell sx={{ whiteSpace: 'nowrap' }}>{inv.item_date}</TableCell>
                      <TableCell>
                        <Typography variant="body2" sx={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={inv.description}>
                          {inv.description || inv.section || '-'}
                        </Typography>
                      </TableCell>
                      <TableCell sx={{ whiteSpace: 'nowrap' }}>
                        <Typography variant="caption" sx={{ color: 'text.secondary' }}>{inv.sub_section || '-'}</Typography>
                      </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={6} sx={{ p: 0, border: 0 }}>
                    <InfiniteScroll loadMore={loadMore} hasMore={hasMore} loading={loading} root={rootEl} threshold={250} />
                  </TableCell>
                </TableRow>
              </>
            )}
