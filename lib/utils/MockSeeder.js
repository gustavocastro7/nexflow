import { connectDB } from '../config/database.js';
import { findOrCreate } from './db.js';
import User from '../models/User.js';
import Workspace from '../models/Workspace.js';
import CostCenter from '../models/CostCenter.js';
import UserWorkspace from '../models/UserWorkspace.js';
import RawInvoice from '../models/RawInvoice.js';
import Invoice from '../models/Invoice.js';
import PhoneLine from '../models/PhoneLine.js';
import Collaborator from '../models/Collaborator.js';

class MockSeeder {
  async run() {
    try {
      await connectDB();

      const [nexflowMatriz] = await findOrCreate(Workspace, { schema_name: 'nexflow_matriz' }, {
        name: 'Nexflow Matriz',
        status: 'active'
      });

      await findOrCreate(CostCenter, { name: 'Matriz', workspace_id: nexflowMatriz.id }, {
        code: 'MATRIZ',
        description: 'Default Cost Center',
      });

      const [teleen] = await findOrCreate(Workspace, { schema_name: 'teleen_consultoria' }, {
        name: 'Teleen Consultoria',
        status: 'active'
      });

      await findOrCreate(CostCenter, { name: 'Matriz', workspace_id: teleen.id }, {
        code: 'MATRIZ',
        description: 'Default Cost Center',
      });

      let jediUsers;
      try { jediUsers = JSON.parse(process.env.JEDI_USERS || '[]'); } catch { jediUsers = []; }

      const seededUsers = [];
      for (const u of jediUsers) {
        const [user] = await findOrCreate(User, { email: u.email }, {
          name: u.name,
          password_hash: u.password,
          profile: u.profile || 'jedi',
          default_workspace_id: nexflowMatriz.id
        });
        seededUsers.push(user);
      }

      for (const user of seededUsers) {
        await findOrCreate(UserWorkspace, { user_id: user.id, workspace_id: nexflowMatriz.id });
        await findOrCreate(UserWorkspace, { user_id: user.id, workspace_id: teleen.id });
      }

      const workspacesData = [
        {
          ws: teleen,
          name: 'Teleen Consultoria',
          schema_name: 'teleen_consultoria',
          costCenters: [
            { code: 'CC-001', name: 'Diretoria', phones: [
              { number: '11900001111', respName: 'Ana Souza', respId: 'EMP-001' },
              { number: '11900002222', respName: 'Ana Souza', respId: 'EMP-001' },
            ] },
            { code: 'CC-002', name: 'IT / Operations', phones: [
              { number: '11900003333', respName: 'Bruno Lima', respId: 'EMP-002' },
              { number: '11900004444', respName: 'Carla Dias', respId: 'EMP-003' },
            ] },
            { code: 'CC-003', name: 'Vendas', phones: [
              { number: '11900005555', respName: 'Diego Alves', respId: 'EMP-004' },
              { number: '11900006666', respName: 'Diego Alves', respId: 'EMP-004' },
            ] },
            { code: 'CC-004', name: 'Financeiro', phones: [
              { number: '11900007777', respName: 'Elisa Rocha', respId: 'EMP-005' },
            ] }
          ],
          users: [],
          claroInvoices: [
            { source_phone: '11900001111', item_date: '2025-02-10', item_time: '10:00:00', description: 'BASIC SERVICE', total_value: 150.50, charged_value: 150.50 },
            { source_phone: '11900003333', item_date: '2025-02-12', item_time: '11:00:00', description: 'MOBILE DATA', total_value: 89.90, charged_value: 89.90 },
            { source_phone: '11900005555', item_date: '2025-02-15', item_time: '09:00:00', description: 'BASIC SERVICE', total_value: 210.00, charged_value: 210.00 },
            { source_phone: '11900001111', item_date: '2025-03-01', item_time: '08:30:00', description: 'BASIC SERVICE', total_value: 120.00, charged_value: 120.00 },
            { source_phone: '11999999999', item_date: '2025-03-03', item_time: '15:00:00', description: 'LOCAL CALL (NOT ALLOCATED)', total_value: 25.00, charged_value: 25.00 }
          ],
          vivoInvoices: [
            { item_date: '2025-02-05', item_time: '14:00:00', source_phone: '11900002222', destination_phone: '11988887777', duration: '00:05:00', description: 'BASE', charged_value: 12.00, total_value: 12.00 },
            { item_date: '2025-02-08', item_time: '16:00:00', source_phone: '11900004444', destination_phone: '11988887777', duration: '00:10:00', description: 'ROAMING', charged_value: 45.00, total_value: 45.00 },
            { item_date: '2025-03-02', item_time: '09:00:00', source_phone: '11900007777', destination_phone: '11911111111', duration: '00:10:00', description: 'SUPORTE VIVO', charged_value: 35.50, total_value: 35.50 },
            { item_date: '2025-03-04', item_time: '10:30:00', source_phone: '11900002222', destination_phone: '11911111111', duration: '00:02:00', description: 'BASE', charged_value: 5.00, total_value: 5.00 }
          ]
        },
      ];

      for (const wsInfo of workspacesData) {
        const ws = wsInfo.ws;

        for (const ccData of wsInfo.costCenters) {
          const phoneNumbers = ccData.phones.map(p => p.number);
          const [cc] = await findOrCreate(CostCenter, { name: ccData.name, workspace_id: ws.id }, {
            code: ccData.code,
            description: `Cost center for ${ccData.name}`,
            phones: phoneNumbers,
          });
          if (!cc.code) { cc.code = ccData.code; await cc.save(); }

          for (const p of ccData.phones) {
            const [collaborator] = await findOrCreate(Collaborator, { name: p.respName, workspace_id: ws.id }, {
              external_id: p.respId,
            });

            await findOrCreate(PhoneLine, { phone_number: p.number, workspace_id: ws.id }, {
              responsible_name: p.respName,
              responsible_id: p.respId,
              collaborator_id: collaborator.id,
              cost_center_id: cc.id,
            });
          }
        }

        const [rawClaro] = await findOrCreate(RawInvoice, { workspace_id: ws.id, operator: 'claro', hash: 'mock_claro' }, {
          content: { info: 'Mock Data' }, processing_status: 'processado'
        });
        const [rawVivo] = await findOrCreate(RawInvoice, { workspace_id: ws.id, operator: 'vivo', hash: 'mock_vivo' }, {
          content: { info: 'Mock Data' }, processing_status: 'processado'
        });

        for (const f of wsInfo.claroInvoices) {
          await findOrCreate(Invoice, {
            source_phone: f.source_phone,
            item_date: f.item_date,
            item_time: f.item_time,
            workspace_id: ws.id,
            operator: 'claro'
          }, {
            raw_invoice_id: rawClaro.id,
            description: f.description,
            total_value: f.total_value,
            charged_value: f.charged_value
          });
        }
        for (const f of wsInfo.vivoInvoices) {
          await findOrCreate(Invoice, {
            source_phone: f.source_phone,
            item_date: f.item_date,
            item_time: f.item_time,
            workspace_id: ws.id,
            operator: 'vivo'
          }, {
            raw_invoice_id: rawVivo.id,
            destination_phone: f.destination_phone,
            duration: f.duration,
            description: f.description,
            charged_value: f.charged_value,
            total_value: f.total_value
          });
        }
      }
    } catch (error) {
      console.error('[MockSeeder] Error:', error);
    }
  }
}

export default new MockSeeder();
