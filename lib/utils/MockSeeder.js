  ...
  async run() {
    try {
      ...
      for (const user of seededUsers) {
        await UserWorkspace.findOrCreate({ where: { user_id: user.id, workspace_id: nexflowMatriz.id } });
        await UserWorkspace.findOrCreate({ where: { user_id: user.id, workspace_id: teleen.id } });
      }
